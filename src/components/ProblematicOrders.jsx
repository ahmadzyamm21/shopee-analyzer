...
const ProblematicOrders = ({ data }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [expandedOrders, setExpandedOrders] = useState({});
  const [problemFilter, setProblemFilter] = useState('all');

  if (!data || !data.problematicOrders) return null;
  ...
  const getFilterBtnStyle = (filter) => {
    ...
    if (isActive) {
      if (filter === 'all') { activeBorder = '1px solid var(--accent-orange)'; activeBg = 'rgba(255,122,26,0.15)'; activeColor = 'var(--accent-orange)'; }
    ...
    <div className="orders-list-container">
      ...
      <div className="card">
        <div className="card-head">
          <h2>📋 Log Pesanan Bermasalah Shopee</h2>
            ...
            <tbody>
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => {
                  ...
                  const totalFeesAmount = order.fees 
                    ? (order.fees.biayaAdmin + order.fees.biayaLayanan + order.fees.biayaTransaksi + order.fees.biayaKomisi + order.fees.biayaProses + order.fees.premi + order.fees.hematKirim + order.fees.biayaKampanye + order.fees.biayaGratongXtra + order.fees.biayaPromoXtra + order.fees.biayaLiveXtra)
                    : 0;
                      ...
                      <tr className="order-row cursor-pointer" onClick={() => toggleExpand(order.id)}>
                        ...
                        <td>
                          {order.resi && order.resi !== '-' ? (
                            <span className="badge shopee-badge" style={{ fontFamily: 'monospace', background: 'rgba(249,115,22,0.1)', color: 'var(--accent-orange)' }}>{order.resi}</span>
                          ) : (
                        ...
                        <td>
                          <span className={`badge ${order.status.toLowerCase().includes('selesai') || order.status.toLowerCase().includes('completed') ? 'green' : order.status.toLowerCase().includes('batal') || order.status.toLowerCase().includes('cancelled') ? 'red' : 'yellow'}`}>
                            {order.status}
                          </span>
                        </td>
                        ...
                        <tr className="expanded-row" style={{ backgroundColor: 'rgba(255,255,255,0.01)' }}>
                          ...
                          <td colSpan="6" style={{ padding: '16px 24px' }}>
                            <div className="expanded-details" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              {/* Reason details */}
                              {(order.cancelReason || order.refund > 0) && (
                                <div style={{
                                  padding: '8px 12px',
                                  borderRadius: '6px',
                                  ...
                                  {order.cancelReason && (
                                    <div>
                                      <span className="text-muted">Alasan Pembatalan: </span>
                              ...
                              <div style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--text-muted2)', fontWeight: 'bold' }}>
                                  <span>BIAYA PLATFORM (SHOPEE):</span>
                                  <span style={{ color: 'var(--accent-orange)' }}>- {formatRp(totalFeesAmount)}</span>
                                </div>
                                {order.fees && (
                                  <>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11.5px', paddingLeft: '8px' }}>
                                      <span className="text-muted">Biaya Admin & Transaksi:</span>
                                      <span>{formatRp(order.fees.biayaAdmin + order.fees.biayaLayanan + order.fees.biayaTransaksi)}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11.5px', paddingLeft: '8px' }}>
                                      <span className="text-muted">Biaya Layanan Program XTRA:</span>
                                      <span>{formatRp(order.fees.biayaGratongXtra + order.fees.biayaPromoXtra + order.fees.biayaLiveXtra)}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11.5px', paddingLeft: '8px' }}>
                                      <span className="text-muted">Logistik & Ongkir Net:</span>
                                      <span>{formatRp(Math.abs(order.fees.netBiayaKirim))}</span>
                                    </div>
                                  </>
                                )}
